 ...
 * 帧格式（小端，共 6 + N 字节）：
 *
 *   偏移  0    1    2     3     4     5 .. 5+N-1   5+N
 *        +----+----+-----+-----+-----+-----------+------+
 *        | A5 | 5A | LEN | SEQ | CMD |  PAYLOAD  | CRC8 |
 *        +----+----+-----+-----+-----+-----------+------+
 *          SOF(2B)  1B    1B    1B      N 字节       1B
 *
 *   整帧长度 = 6 + N 字节（下标 0 ~ 5+N）。
 *   注意别把"整帧长度 6+N"当成"CRC 的下标"：载荷占 5 ~ 5+N-1，
 *   所以 CRC 落在下标 5+N，也就是整帧的最后一个字节（6+N-1）。
 *   这个差 1 的坑在测试里也踩过（曾经写成 frame[10] 找载荷最后一字节）。
 *
...
typedef struct PROTO_PACKED {
    ...
    uint8_t crc;        /* 偏移 5+N：校验，只在校验通过后才有意义（整帧最后一个字节） */
} ProtoFrame;
